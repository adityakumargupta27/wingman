...
export function startTelegramBot(handlers) {
  ...
  bot.on("message", async msg => {
    ...
    const isPaused = false; // Set to true to pause the bot
    if (isPaused) {
  ...
  bot.on('polling_error', async (err) => {
    ...
    const isFatal = err.message.includes('EFATAL') ||
                    err.message.includes('ENOTFOUND') ||
                    err.message.includes('ECONNRESET') ||
                    err.message.includes('ECONNREFUSED') ||
                    err.message.includes('ETIMEDOUT');

    if (isFatal && !isRecovering) {
      isRecovering = true;
      recoveryAttempts++;
      const delay = Math.min(10000 * Math.pow(2, recoveryAttempts - 1), 120000); // 10s → 20s → 40s → max 2min

      log.error(`Telegram FATAL polling error — auto-recovering in ${delay / 1000}s (attempt ${recoveryAttempts})`, {
        error: err.message,
      });
      ...
      setTimeout(async () => {
        try {
          log.info(`Telegram auto-recovery: restarting polling (attempt ${recoveryAttempts})`);
          await bot.startPolling();
          recoveryAttempts = 0; // Reset on success
          log.info('Telegram polling recovered successfully ✅');
        } catch (restartErr) {
          log.error('Telegram recovery failed — will retry', { error: restartErr.message });
        } finally {
          isRecovering = false;
        }
      }, delay);
    ...
    // If isFatal && isRecovering, silently ignore (recovery already in progress)
  });
