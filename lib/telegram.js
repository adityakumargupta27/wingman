...
import { classifyIntent } from './prompt-router.js';
...
// Stores pending intent (e.g., 'tailor') for users who sent a command without args
...
function launchBot(token) {
  ...
  bot.on('message', async (msg) => {
    ...
    // 1. Exact Command Match (Explicitly mapped)
    if (text.startsWith('/')) {
      pendingCommands.delete(userId);
      const cmd = text.split(' ')[0].toLowerCase();
      const payload = text.slice(cmd.length).trim();

      if (cmd === '/project' || cmd === '/proj')   return safeHandle(chatId, () => handleProject(chatId, payload));
      if (cmd === '/evaluate' || cmd === '/eval') return safeHandle(chatId, () => handleEvaluate(chatId, userId, payload));
      if (cmd === '/deep' || cmd === '/research') return safeHandle(chatId, () => handleDeep(chatId, payload));
      if (cmd === '/tailor')                       return safeHandle(chatId, () => handleTailor(chatId, userId, payload));
      if (cmd === '/jobs')                         return safeHandle(chatId, () => handleJobs(chatId, userId, payload));
      if (cmd === '/tracker')                      return safeHandle(chatId, () => handleTracker(chatId, userId));
      
      if (cmd === '/start' || cmd === '/help')     return handleHelp(chatId);
      if (cmd === '/cancel')                       return bot.sendMessage(chatId, "✅ Action cancelled.");
      
      return bot.sendMessage(chatId, "⚠️ Unknown command. Use /help to see available features.");
    }

    // 2. Intent Classifier (For URLs and freeform text)
    const { route, confidence, extractedData } = classifyIntent(text);
    const url = extractedData.url || '';

    if (confidence === 'url') {
      if (route?.id === 'project') return safeHandle(chatId, () => handleProject(chatId, url));
      return safeHandle(chatId, () => handleEvaluate(chatId, userId, url));
    }

    if (confidence === 'keyword') {
      if (route?.id === 'project') return safeHandle(chatId, () => handleProject(chatId, extractedData.payload));
      if (route?.id === 'deep')    return safeHandle(chatId, () => handleDeep(chatId, extractedData.payload));
      if (route?.id === 'tailor')  return safeHandle(chatId, () => handleTailor(chatId, userId, extractedData.payload));
    }
...
async function handleProject(chatId, description) {
  if (!description) {
    return bot.sendMessage(chatId, "🚀 Please provide a project description or GitHub URL.\nUsage: `/project I built a React app...`", { parse_mode: 'Markdown' });
  }
  ...
  try {
    ...
    // Auto-scrape if it's a URL
    if (description.match(/^https?:\/\//)) {
      await bot.editMessageText(`🔍 Scraping repository data...`, { chat_id: chatId, message_id: statusMsg.message_id });
      try {
        const scraped = await fetchJobDescription(description);
        context = `URL: ${description}\n\nCONTENT:\n${scraped.text}`;
      } catch (e) {
        log.warn('Project scrape failed, using URL only', { error: e.message });
      }
    }

    await bot.editMessageText(`🧠 Extracting hidden skills and metrics...`, { chat_id: chatId, message_id: statusMsg.message_id });
    
    const systemPrompt = buildProjectPrompt();
    const response = await analyzeProject({ systemPrompt, description: context });
