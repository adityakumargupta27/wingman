...
function launchBot(token) {
  ...
  bot.on('message', async (msg) => {
    ...
    if (text === '/jobs') {
      return safeHandle(chatId, () => handleJobs(chatId, userId));
    }
...
async function handleJobs(chatId, userId) {
  ...
  try {
    const cvText = await getCV(userId);
    // ISOLATED: Dedicated job hunter prompt, CV in system, request in user
    const systemPrompt = buildJobHunterPrompt(cvText);
    const response = await callGemini(systemPrompt, "Find me the best jobs/internships matching my profile right now.");

    await bot.sendMessage(chatId, `🏹 *Smart Job Matches*\n\n${response.slice(0, 4000)}`, { parse_mode: 'Markdown' });
  } catch (err) {
