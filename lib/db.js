...
function queryAll(sql, params = []) {
  const sanitizedParams = params.map(x => x === undefined ? null : x);
...
export function saveApplication({ discordId, company, role, score, archetype, legitimacy, jdSnippet, reportText, storiesJson, threadId }) {
  const params = [discordId, company, role, score, archetype, legitimacy, jdSnippet, reportText, storiesJson, threadId].map(x => x === undefined ? null : x);
  db.run(`
