...
import { generateResumePDF } from '../utils/pdfBuilder.js';
...
export const handlers = {
  handleHelp: async (chatId, h) => {
    if (!h) {
      log.error('handleHelp called without handler object');
      return;
    }
  ...
  handleProject: async (chatId, description, h) => {
    ...
    try {
      ...
      if (description.includes('github.com')) {
        ...
        log.warn("GitHub API analysis failed, falling back to standard project analysis", { error: result.error, description });
      }
  ...
  handleTailor: async (chatId, userId, input, h) => {
    try {
      ...
      // Check if we already have a choice in the input (from callback query)
      const formatMatch = input.match(/^format:(pdf|word)\|jd:(.+)$/);
      
      if (!formatMatch) {
        // First step: Ask for format
        return h.send(chatId, "📄 Which format would you like your tailored resume in?", {
          reply_markup: {
            inline_keyboard: [[
              { text: '📄 PDF Format', callback_data: `tailor_format:pdf|jd:${input.slice(0, 40)}` },
              { text: '📝 Word Format', callback_data: `tailor_format:word|jd:${input.slice(0, 40)}` }
            ]]
          }
        });
      }
      ...
      const jdInput = formatMatch[2];

      let jdText = jdInput;
      let role = 'Target Role';
      let company = 'Target Company';
      
      if (jdInput.match(/^https?:\/\//)) {
        await h.send(chatId, `🔍 Scraping job description...`);
        const scraped = await fetchJobDescription(jdInput);
        jdText = scraped.text;
      ...
      await h.send(chatId, `🧠 Tailoring resume in ${format.toUpperCase()} — this may take a moment...`);
      const systemPrompt = buildTailorPrompt({ role, company, jd: jdText });
      const response = await tailorResume({ systemPrompt, cvText });

      if (format === 'pdf') {
        const pdfBuffer = await generateResumePDF(response);
        await h.bot.sendDocument(chatId, pdfBuffer, {
          caption: '✨ Your ATS-tailored resume (PDF)!',
        }, {
          filename: 'Wingman_Tailored_Resume.pdf',
          contentType: 'application/pdf',
        });
      } else {
        const wordBuffer = await generateResumeWord(response);
        await h.bot.sendDocument(chatId, wordBuffer, {
          caption: '✨ Your ATS-tailored resume (Word)! You can easily edit this one.',
        }, {
          filename: 'Wingman_Tailored_Resume.docx',
      ...
      log.error('TAILOR ERROR:', { error: err.message, stack: err.stack });
  ...
  handleJobs: async (chatId, userId, filter, h) => {
    try {
      ...
      // Force refresh profile if CV exists but profile is stale or missing
      if (!profile || (cvText && profile.updated_at < (Date.now() / 1000) - 3600)) {
        log.info('Refreshing candidate profile for recommendations', { userId });
        profile = buildCandidateProfile(cvText || '');
        upsertCandidateProfile(userId, profile);
      }

      if (!filter) {
        // Ask the user to choose a location filter
        return h.send(chatId, "📍 *Where are you looking for jobs?*\n\nSelect an option below or type `/jobs [location]` (e.g. `/jobs Bangalore, Remote`)", {
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: [
              [{ text: '🌍 Remote Only', callback_data: 'jobs_loc_remote' }],
              [{ text: '🇮🇳 Bangalore', callback_data: 'jobs_loc_bangalore' }, { text: '🇮🇳 Hyderabad', callback_data: 'jobs_loc_hyderabad' }],
              [{ text: '🇮🇳 Delhi/NCR', callback_data: 'jobs_loc_delhi' }, { text: '🇮🇳 Pune', callback_data: 'jobs_loc_pune' }],
              [{ text: '🌐 Anywhere (All Jobs)', callback_data: 'jobs_loc_all' }]
            ]
          }
        });
      }
      ...
      if (filter && filter.toLowerCase() !== 'all') {
        locs = filter.split(',').map(l => l.trim()).filter(Boolean);
        await h.send(chatId, `📍 Filtering jobs for: *${locs.join(', ')}*...`, { parse_mode: 'Markdown' });
      } else {
        await h.send(chatId, `🌐 Searching all locations...`);
      }

      // Increase pool to 500 for better matching variety
      const jobs = getActiveJobs(500, locs);
      if (!jobs.length) {
        return h.send(chatId, `📡 No jobs found${locs.length ? ' for those locations' : ''}. Try again with different filters.`);
      }

      const ranked = scoreJobs(jobs, profile, 7);
      if (!ranked.length) {
        return h.send(chatId, 
          "🏹 *No matches found for your current profile.*\n\n" +
          "💡 *Tips to improve recommendations:*\n" +
          "• Upload a detailed **PDF Resume**\n" +
          "• Use **/project** to add technical DNA\n" +
          "• Try a broader search keyword (e.g., `/jobs react`)"
        );
      }
  ...
  handleDocument: async (chatId, userId, document, h) => {
    try {
      ...
      await h.send(chatId, `📥 Processing *${file_name}*...`);
      ...
      if (text.length < 50) {
        return h.send(chatId, "⚠️ The PDF appears empty or image-only. Please upload a resume with selectable text.");
      }

      // 4. Ensure user row exists, THEN save CV
      upsertUser(userId, `tg_${userId}`);
      setCV(userId, text);
      
      log.info('Resume uploaded via Telegram', { userId, chars: text.length, pages: data.numpages });

      return h.send(chatId, 
        `✅ *Resume Stored!* (${text.length} chars, ${data.numpages} page${data.numpages > 1 ? 's' : ''})\n\n` +
        `You can now:\n` +
        `• Paste a *Job URL* to evaluate fit\n` +
        `• Use */tailor* to ATS-optimize your CV\n` +
        `• Use */jobs* for smart recommendations\n` +
        `• Just chat for career advice!`
      );

    } catch (err) {
      log.error('DOCUMENT ERROR:', { error: err.message, stack: err.stack, userId });
      return h.send(chatId, `❌ PDF processing failed: ${err.message}`);
    }
  }
};
