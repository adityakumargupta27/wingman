...
export function buildCandidateProfile(cvText, preferences = {}) {
  ...
  const TECH_SKILLS = [
    ...
    'django', 'flask', 'spring', 'rails', 'fastapi',
    'postgresql', 'mysql', 'mongodb', 'redis', 'elasticsearch', 'dynamodb',
    'aws', 'gcp', 'azure', 'docker', 'kubernetes', 'terraform',
    'graphql', 'rest', 'kafka', 'rabbitmq',
    'pytorch', 'tensorflow', 'langchain', 'openai', 'machine learning',
    'figma', 'tailwind', 'css', 'html', 'git', 'linux', 'sql',
...
export function scoreJob(job, profile) {
  ...
  if (jobSkills.length === 0) {
    components.skill = 5; // Unknown, neutral
  } else {
    const overlap = jobSkills.filter(s => userSkills.includes(s)).length;
    components.skill = Math.min(10, Math.round((overlap / jobSkills.length) * 10));
  }
