...
export async function scanInbox(discordId) {
  ...
  try {
    ...
    // If the refresh token has been revoked or expired, clear it so we don't
    // keep retrying a dead token on every scout cycle.
    if (err.message?.includes('invalid_grant') || err.code === 401) {
      log.warn('Gmail refresh token revoked/expired — clearing stored token', { discordId });
      saveGmailToken(discordId, null);
