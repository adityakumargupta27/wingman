...
async function runScout(client) {
  ...
  for (const user of users) {
    try {
      ...
      if (userErr.message === 'TOKEN_REVOKED') {
        log.warn(`Scout: Gmail token revoked for ${user.username} — skipping until re-authorized`);
      } else if (userErr.message === 'NOT_AUTHORIZED') {
        log.info(`Scout: No Gmail token for ${user.username} — skipping`);
      } else {
