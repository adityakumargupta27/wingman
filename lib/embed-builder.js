/**
 * lib/embed-builder.js — Discord Embed factory for Wingman
 *
...
export function buildEvalEmbed({ company, role, score, archetype, legitimacy }) {
  ...
  return new EmbedBuilder()
    ...
    .setThumbnail('https://cdn-icons-png.flaticon.com/512/1063/1063376.png')
    .setDescription(`**${role}**`)
    .addFields(
    ...
    .setFooter({ text: 'Wingman AI · Career Intelligence' })
    .setTimestamp();
}
...
export function buildTrackerEmbed(applications, username) {
  ...
  const rows = applications.map(a => {
    ...
    return `${emoji} **#${a.id}** | ${a.company} — ${a.role}\n    Score: \`${score}/10\` · Status: ${a.status} · ${date}`;
  });
...
export function buildHelpEmbed() {
  return new EmbedBuilder()
    .setColor(COLORS.info)
    .setTitle('🤖 Wingman — Career Intelligence Assistant')
    .setDescription('Your AI-powered career wingman. Evaluate jobs, prep for interviews, and track your pipeline — all inside Discord.')
    .addFields(
      {
        name: '📋 Evaluation',
        value: [
    ...
    .setFooter({ text: 'Wingman · Use /evaluate to get started' });
}
