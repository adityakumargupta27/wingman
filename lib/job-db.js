...
export function upsertJobs(jobs) {
  ...
  for (const job of jobs) {
    try {
      ...
      log.warn('Failed to upsert job', { source_id: job.source_id, error: err.message });
    }
  }
  ...
  log.info('Jobs upserted', { total: jobs.length, inserted, updated });
  return inserted;
}
 ...
 */
export function getActiveJobs(limit = 2000, locations = []) {
  ...
  if (locations && locations.length > 0) {
    const locConditions = locations.map(loc => {
      ...
      return `LOWER(location) LIKE ?`;
    }).join(' OR ');
    sql += ` AND (${locConditions})`;
  }
