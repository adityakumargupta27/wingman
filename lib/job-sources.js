...
const GREENHOUSE_BOARDS = [
  ...
  'unacademy', 'upstox', 'phonepe', 'bharatpe', 'cars24',
...
/**
 * Fetch jobs from LinkedIn Public Search (simulated via aggregator endpoint)
 */
async function fetchLinkedInJobs() {
  try {
    // LinkedIn doesn't have a clean public JSON API, 
    // so we simulate the extraction pattern here for the pipeline.
    // In production, this would use Playwright or an aggregator service API to scrape LinkedIn.
    log.info('Fetching latest jobs from LinkedIn...');
    return []; 
  } catch (err) {
    log.warn(`LinkedIn fetch failed: `, { error: err.message });
    return [];
  }
}
