...
const GREENHOUSE_BOARDS = [
  ...
  // New Global Tech
  'uber', 'lyft', 'doordash', 'instacart', 'pinterest',
  'dropbox', 'asana', 'snowflake', 'palantir',
  'robinhood', 'rippling', 'deel',
  // New Indian Startups & Unicorns
  'razorpayindia', 'unacademy', 'upstox', 'phonepe', 'bharatpe', 'cars24',
  'ola', 'oyo', 'byjus', 'dream11', 'ofbusiness',
  'lendingkart', 'cleartax', 'urbancompany', 'curefit', 'dunzo',
  'lenskart', 'nykaa', 'pharmeasy', 'policybazaar', 'spinny',
  'tata1mg', 'upgrad', 'vedantu', 'zepto', 'zoho',
  'browserstack', 'clevertap', 'chargebee', 'freshworks', 'mindtickle',
  'hasura', 'druva', 'inmobi', 'glance',
];
...
const LEVER_ACCOUNTS = [
  ...
  // New Indian & Global
  'kpmg', 'coursera', 'wework', 'revolut',
  'shopclues', 'pine-labs', 'myntra', 'flipkart', 'paytm',
  'makemytrip', 'bookmyshow', 'fractal', 'greyorange', 'highradius',
  'cred', 'meesho',
];
...
const ASHBY_BOARDS = [
  ...
  // New AI & Modern Tech
  'apollo', 'jasper', 'anthropic', 'midjourney',
  'huggingface', 'weightsandbiases', 'scale', 'snakel',
];
...
// ── Added Platforms: Unstop & Internshala ────────────────────────────────────

/**
 * Fetch jobs from Unstop public search API
 */
async function fetchUnstopJobs() {
  try {
    // Note: Unstop uses a POST payload for search in some API versions,
    // this acts as a robust stub for the ingestion engine to parse their opportunities
    const url = `https://unstop.com/api/public/opportunity/search-result?opportunity=jobs&page=1`;
    const res = await fetchWithTimeout(url);
    if (!res.ok) return [];
    ...
    return (data?.data?.data || []).map(j => ({
      ...
      title: j.title,
      location: j.jobLocation || 'Unknown',
      url: `https://unstop.com/${j.seo_url}`,
      description: stripHtml(j.details || ''),
      skills: extractSkillsFromJD(j.details || ''),
      experience_min: inferMinExperience(j.title, j.details || ''),
      experience_max: inferMaxExperience(j.title, j.details || ''),
      employment_type: inferEmploymentType(j.title),
      posted_at: j.start_date || new Date().toISOString(),
    }));
  } catch (err) {
    log.warn(`Unstop fetch failed: `, { error: err.message });
    return [];
  }
}

/**
 * Fetch jobs from Internshala (simulated via RSS/JSON or HTML parsing)
 */
async function fetchInternshalaJobs() {
  try {
    // Internshala doesn't have a clean public JSON API, 
    // so we simulate the extraction pattern here for the pipeline
    // In production, this would use the Playwright scraper.js on their search page.
    log.info('Fetching latest jobs from Internshala...');
    // Return empty array for now until full HTML parsing is implemented
    return []; 
  } catch (err) {
    log.warn(`Internshala fetch failed: `, { error: err.message });
    return [];
  }
}

/**
 * Fetch jobs from LinkedIn Public Search using Playwright
 */
async function fetchLinkedInJobs() {
  let browser;
  try {
    log.info('Scraping LinkedIn for Software Engineer jobs in India...');
    browser = await chromium.launch({ headless: true });
    const page = await browser.newPage();
    
    // Use LinkedIn's public job search URL
    const searchUrl = 'https://www.linkedin.com/jobs/search?keywords=Software%20Engineer&location=India&f_TPR=r86400';
    ...
    const jobs = await page.evaluate(() => {
      ...
      return cards.map(card => {
        ...
        return {
          source: 'linkedin',
          source_id: `li_${linkEl?.href.split('?')[0].split('/').pop() || Math.random().toString(36).slice(2)}`,
          ...
          description: 'Apply via LinkedIn',
          skills: [],
          experience_min: 0,
          experience_max: 5,
          employment_type: 'full-time',
          posted_at: dateEl?.getAttribute('datetime') || new Date().toISOString(),
        };
      }).filter(j => j.url);
    });
    
    log.info(`LinkedIn scrape successful: Found ${jobs.length} jobs`);
    return jobs;
  } catch (err) {
    log.warn(`LinkedIn scrape failed: ${err.message}`);
    return [];
  } finally {
