...
export async function callGemini(systemPrompt, userContent, modelName = DEFAULT_MODEL) {
  ...
  // Try with primary model, then fallback if model not found
  ...
  for (const currentModel of modelsToTry) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Model not found — try fallback model immediately
        if (category === 'MODEL_NOT_FOUND') {
          log.info(`Model ${currentModel} not found, trying fallback...`);
          break; // Break inner loop, continue outer loop to try fallback model
        }
    ...
    // If we're here and this wasn't the last model, continue to fallback
...
function friendlyMessage(category, err) {
  switch (category) {
    ...
    case 'MODEL_NOT_FOUND':
      return '🔄 AI model not available — switching to fallback. Please retry.';
    case 'RATE_LIMIT':
